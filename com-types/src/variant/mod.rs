...
pub mod sys;
use self::sys::{VariantInit, VariantClear, VariantCopy, VariantCopyInd};
...
#[repr(C)]
pub struct Tag(u16);
impl_inner_newtype!(Tag: u16);
use ::array::ElemTag;
impl From<ElemTag> for Tag {
	fn from(elem_tag: ElemTag) -> Tag {
		use ::IntoInner;
		Tag(elem_tag.into_inner() | vt::VT_ARRAY)
	}
}
